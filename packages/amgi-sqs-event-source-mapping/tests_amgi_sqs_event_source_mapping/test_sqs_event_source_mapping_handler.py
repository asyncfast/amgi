...
import boto3
import pytest
from amgi_sqs_event_source_mapping import _SqsEventSourceMapping
...
async def app_sqs_handler() -> (
    ...
    call_task = loop.create_task(sqs_handler._call({"Records": []}, Mock()))
...
async def test_lifespan() -> None:
    ...
    lifespan_task = loop.create_task(sqs_handler._call({"Records": []}, Mock()))
...
async def test_sqs_event_source_mapping_handler_invocation_hook() -> None:
    app = MockApp()
